...
from pathfinding.roadpaths import BRIDGE, ROAD, ROAD_SLAB, create_get_neighbours_function, get_cost
...
class BubbleGenerator(Generator):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.hmap = self.slice.heightmaps['MOTION_BLOCKING_NO_LEAVES']
    ...
    def __generate__(self, interface : Interface):
        ...
        for x in range(self.width):
            for z in range(self.depth):
                interface.placeBlock(x, self.hmap[x][z], z, self.get_bubble_color(bubble_map[x][z]))
    ...
    def build_path(self, path, interface):
        for point in path:
            ...
            interface.placeBlock(point[0], point[1], point[2], block)
    ...
    def generate_roads(self, interface):
        ...
        for bubble in self.bubbles:
            ...
            y = self.hmap[int(x)][int(z)]
            points.append((int(x), y, int(z), ROAD, 0))

        wmap = get_water_map(interface, self.hmap)

        web = get_web(
            points, 
            create_get_neighbours_function(self.width, self.depth, self.hmap, interface, wmap), 
            get_cost
        )
